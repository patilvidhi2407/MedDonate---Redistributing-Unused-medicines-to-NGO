                ...
                if (isset($_POST['accept'])) {
                    ...
                    if (isset($_POST['id']) && is_array($_POST['id'])) {
                        foreach ($_POST['id'] as $id) {
                            $sql = "UPDATE donar3 SET status='Accepted' WHERE id='$id'";
                            $conn->query($sql);
                        }
                    }
                    ...
                    if (isset($_POST['id']) && is_array($_POST['id'])) {
                        foreach ($_POST['id'] as $id) {
                            $sql = "UPDATE donar3 SET status='Rejected' WHERE id='$id'";
                            $conn->query($sql);
                        }
                    }
                }
           ...
           header("Location: req.php");
            ...
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    echo "<tr>";
                    echo "<td><input type='checkbox' name='id[]' value='" . $row['id'] . "' class='chkboxname'></td>";
                    echo "<td>" . $row['id'] . "</td>";
